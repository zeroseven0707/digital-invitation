...
class PublicInvitationController extends Controller
{
    ...
    public function show(string $uniqueUrl, Request $request)
    {
        ...
        // Resolve guest by QR token (?to=qr_token) — optional
        $guestForQr = null;
        $qrToken = $request->query('to');
        if ($qrToken) {
            $guestForQr = $invitation->guests()
    ...
    /**
     * Display the invitation for mobile app (JSON response).
     *
     * @param string $uniqueUrl
     * @param Request $request
     * @return \Illuminate\Http\JsonResponse
     */
    public function showMobile(string $uniqueUrl, Request $request)
    {
        // Find invitation by unique URL and ensure it's published
        $invitation = Invitation::where('unique_url', $uniqueUrl)
            ->where('status', 'published')
